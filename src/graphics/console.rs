...
impl ColoredChar {
    pub fn new(id: u32, fg_color: [u8; 3], bg_color: [u8; 3]) -> Self {
        ColoredChar {
            id,
            fg_color,
            bg_color,
...
implement_vertex!(Vertex, position, tex_position, fg, bg, overlay);
...
impl Root {
    ...
    pub fn put_char(&mut self, char: u32, pos: (u32, u32)) {
    ...
    pub fn draw(&mut self) {
        ...
        let glyph_size =
            self.display.gl_window().get_inner_size().unwrap().width as f32 / self.size.0 as f32;
        let glyph_scale =
            glyph_size / self.display.gl_window().get_inner_size().unwrap().width as f32 * 2.0;
        self.vertexes.clear();
